...
if(isset($_POST['manage'])){
	...
	$accept=$_POST['accept'];
	//print_r($dua);
	mysqli_query($con, "UPDATE job_scope SET job_status='$accept' WHERE jobscope_id='$id'");
	$msgBox=alertBox('List Updated.');
}
if(isset($_POST['manage2'])){
	$id=$_POST['id'];
	$decline=$_POST['decline'];
	//print_r($dua);
	mysqli_query($con, "UPDATE job_scope SET job_status='$decline' WHERE jobscope_id='$id'");
	$msgBox=alertBox('List Updated.');
}
?>
...
<body id="page-top">
  ...
  <!-- Add User for the Activity Proposal -->
            ...
            <div class="row">            
            	...
            	<?php if($msgBox){ echo $msgBox; } ?>
					...
					<div class="card-header py-3">
					  <h6 class="m-0 font-weight-bold text-primary">Job Scope <em>Jawatankuasa Pelaksana - <?php echo $user; ?></em></h6>
					</div>
					<div class="container">
					  <p></p>
						  ...
						  <tbody>
							...
							$join=mysqli_query($con, "SELECT * FROM job_scope WHERE id_orgDetail='$IDORG'");
							  while($col = mysqli_fetch_array($join)){
								  ...
								  $idjob=$col["jobscope_id"];
								  $job_status=$col["job_status"];
							...
							<tr>
							  ...
							  <td>
								  ...
								  if($job_status==0){
									  ?>
							  	  <a href="#" class="" data-toggle="modal" data-target="#updateStatus<?php echo $idjob; ?>"><span class="btn btn-secondary btn-group-sm fa fa-save" data-toggle="tooltip" data-placement="left" title="" data-original-title="Incompleted"> Incompleted</span></a>
								  	  
									  
									  <?php
								  }elseif($job_status==1){
									  ?>
								  	 <a href="#" class="" data-toggle="modal" data-target="#updateStatus<?php echo $idjob; ?>"><span class="btn btn-success btn-group-sm fa fa-save" data-toggle="tooltip" data-placement="left" title="" data-original-title="Completed"> Completed</span></a>
	...
	<div class="modal fade" id="updateStatus<?php echo $idjob; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div id="largeModal" class="modal-dialog modal-lg" role="document">
      ...
      <div class="modal-header">
   	...
   	<input type="hidden" name="id" value="<?php echo $idjob; ?>">
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-primary" type="accept" name="manage">Completed</button>
		  <button class="btn btn-danger" type="decline" name="manage2">Incompleted</button>
          </form>
        </div>
      </div>
    </div>
  ...
  <?php } ?>
		...
		<div class="col-lg-12">
			<div style="float: right;">
				<a class="btn btn-success btn-lg" href="activityMonitoring-list-view.php?id=<?php echo $proposalId; ?>&amp;username=<?php echo $userJawatan; ?> " id="btnBack"><i class="fa fa-backward"></i> Back</a>
