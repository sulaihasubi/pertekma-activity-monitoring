...
if(isset($_POST['submiting'])){
	...
	$time=$_POST['time'];
	$changetime=date("Y-m-d H:i:s", strtotime($time));
	$activityLocation=$_POST['activityLocation'];
	$objective=$_POST['objective'];

	for($i=0;$i<count($activityTitle);$i++){
		if($proID!="" && $proposal[$i]!="" && $stake[$i]!=""){
			$check_activity = mysqli_query($con, "SELECT * FROM activity WHERE proposald = '$proID' AND activityTitle = '$activity[$i]'");
			if(mysqli_num_rows($check_activity) <= 0){
   				mysqli_query($con, "INSERT INTO activity (activityTitle, date, time, activityLocation, objective) VALUES('$proID','$activityTitle[$i]','$date[$i]','$time[$i]','$activityLocation[$i]','$objective[$i]')");
				ob_start();
				header('Location:activity-add.php?id='.$proID);
			}else{
				$msgBox = alertBox('Activity sucessfully added.');
				
			}	
  		}else{
				$msgBox = alertBox('Activity Title already existed. Please input another activity.');}
 	}		
}
?>
                		...
                		<div class="entry input-group">
							<div class="col-lg-12">
								<div class="form-group row">
									<label for="inputEmail3" class="col-sm-2 col-form-label">Tajuk</label>
									<div class="col-sm-10">
									<input type="name" class="form-control" id="inputName3" placeholder="Title">
									</div>
								</div>
								
								<div class="form-group row">
									<label for="inputq" class="col-sm-2 col-form-label">Tarikh Mula</label>
									<div class="col-sm-10">&nbsp;&nbsp;&nbsp;
								...
								<div class="form-group row">
									<label for="inputEmail3" class="col-sm-2 col-form-label">Tarikh Tamat</label>
									<div class="col-sm-10">&nbsp;&nbsp;&nbsp;
								...
								<div class="form-group row">
									<label for="inputEmail3" class="col-sm-2 col-form-label">Masa</label>
									<div class="col-sm-10 row">&nbsp;&nbsp;&nbsp;
										<select name="vStart" class="form-control col-sm-3">
										   ...
										   <label for="inputEmail3" class="col-form-label">to</label>
											&nbsp;
										   <select name="vEnd" class="form-control col-sm-3">
										   <option value="7:00 a.m">7:00 a.m</option>
										   <option value="7:30 a.m">7:30 a.m</option>
