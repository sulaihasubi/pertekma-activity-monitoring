...
<?php
$proposalId=$_GET["id"];
//$acaSessionId=$_GET["idacaSession"];
...
if(isset($_POST['submitin'])){
	...
	for($i=0;$i<count($pronama);$i++){
		...
		if($pronama!="" && $proposition1[$i]!="" || $proposition2[$i]!=""){
			if($proposition1[$i]!=""){
				// kurang idporposal
				mysqli_query($con, "INSERT INTO org_detail ( nama, ja_watan) VALUES('$pronama[$i]','$proposition1[$i]')");
			}elseif($proposition2[$i]!=""){
				mysqli_query($con, "INSERT INTO org_detail ( nama, ja_watan) VALUES('$pronama[$i]','$proposition1[$i]')");
				//mysqli_query($con, "INSERT INTO org_detail ( idOrganisasi, nama, ja_watan) VALUES( '$idOrganisasi', '$pronama[$i]')");
				
				//mysqli_query($con, "INSERT INTO acasessdetail ( acaSessionId, nama, jawatan, noMatric, noTel) VALUES( '$acaSessionId', '$pronama[$i]', '$projawatan[$i]','$pronoMatric[$i]', '$pronoTel[$i]')");
				
				// Check and insert into database
				$check_position = mysqli_query($con, "SELECT * FROM organisasi WHERE position = '$proposition2[$i]'");
				if(mysqli_num_rows($check_position) == 0){
					mysqli_query($con, "INSERT INTO organisasi(position) VALUES ('$proposition2[$i]')");
				}
				
			}
			//header('Location:equipment.php?id='.$proposalId);
			//header('Location:acaSession-add.php?id='.$acaSessionId);
  		}else{
     ...
     <!-- Nav Item - Activity Pages  Menu --> 
      <li class="nav-item ">
        ...
        <div id="collapseActivity" class="collapse" aria-labelledby="headingActivity" data-parent="#accordionSidebar">
          <div class="bg-white py-2 collapse-inner rounded">
            <a class="collapse-item" href="activityMonitoring.php">Activity Monitoring</a>
            ...
            <div class="row">
                 <div class="col-md-3 col-lg-12">
                    <div class="form-group">
                      <label class="control-label"><b>10.0 Jawatankuasa Pelaksana</b></label>
                    </div>
                 </div>
				
				 <div class="form-group row col-lg-12">
						<label for="inputproposition1" class="col-sm-2 col-form-label">Jawatan</label>
						<div class="controls col-sm-10">
						 <select name="proposition1">
						 <option  value="">Please Choose</option>
						 <?php
							$cdquery = "SELECT * FROM organisasi ";
							$cdresult = mysqli_query($con, $cdquery);

							while ($cdrow = mysqli_fetch_array($cdresult))
								{
								$position = $cdrow["position"];
								echo "<option ='$position'>$position</option>";
								}
							?>
						</select>
							
						<input type="text" name="proposition2" placeholder="Fill here if not in the list"/>
						</div>
					 <label for="inputproposition2" class="col-sm-2 col-form-label"></label>
						<div class="col-sm-10">
							<input type="text" name="proposition2" class="form-control" id="inputproposition2" placeholder="Jawatan Ketua Program">*Ruangan ini dikhaskan untuk Jawatan Ketua Program yang tidak tersenarai.
						</div>
					</div>	
                		...
                		<div class="entry input-group">
							<div class="col-lg-12">
								...
								<div class="form-group row">
									...
									<div class="col-sm-10">
										<input type="text" name="pronama[]" class="form-control" id="inputPassword3" placeholder="Nama Penuh">
									</div>
								</div>
 ...
 <div class="row">
    <div class="col-lg-12">
        <div style="float: right;">
            <a class="btn btn-success btn-lg" href="percubaan-.php?id=<?php echo $proposalId; ?>" id="btnBack"><i class="fa fa-backward"></i> Back</a>
